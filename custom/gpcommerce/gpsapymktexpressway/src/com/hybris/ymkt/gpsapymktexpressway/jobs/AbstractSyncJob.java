...
import com.hybris.ymkt.gpsapymktexpressway.services.AbstractSyncService;
import com.hybris.ymkt.gpsapymktexpressway.services.ImportMessageService;
import com.hybris.ymkt.gpsapymktexpressway.services.SyncChangeDetectionService;
		...
		extends AbstractJobPerformable<T>
{
	protected class BatchBuilder implements Iterable<List<SyncJobBatchEntry<M>>>, Iterator<List<SyncJobBatchEntry<M>>>
	{
		...
		protected boolean populateModel(final SyncJobBatchEntry<M> entry)
		{
			...
			LOG.error("Unable to read nor create ItemModel from '{}' because of {}. ItemChangeDTOs will be ignored and consumed.",
	...
	protected Map<String, Object> getSendParameters(final T cronJob)
	{
		...
		LOG.debug("Using site ={}", cronJob.getSiteId());
		parameters.put(GpsapymktexpresswayConstants.SITE_ID, cronJob.getSiteId());
	...
	public PerformResult perform(final T cronJob)
	{
		...
		final BatchBuilder batch = new BatchBuilder(changes.values(), this.getBatchSize(cronJob));
