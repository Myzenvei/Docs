...
public class DefaultLegacyValueProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider, Serializable
{
			...
			final Object model) throws FieldValueProviderException
	{
		...
		if(model instanceof PriceRowModel) {
			product = ((PriceRowModel)model).getProduct();
			if (product == null)
			{
				throw new IllegalArgumentException("Product not found in pricerow");
			}
		}
		else {
			throw new FieldValueProviderException("Cannot evaluate value of non-priceRow item");
		}
		...
		if (indexedProperty.isContextual() && !indexedProperty.isLocalized())
		{
			for (final ContextualAttributesContextModel localContex : indexConfig.getContexts())
			{
				...
				final Object value = getValueWhenContextual(indexedProperty, product, localContex);
			...
			for (final ContextualAttributesContextModel localContex : indexConfig.getContexts())
			{
				...
				for (final LanguageModel language : languages)
				{
					...
					try
					{
						...
						value = getValueWhenContextual(indexedProperty, product, localContex);
						rangeNameList = getRangeNameList(indexedProperty, value);
					}
					finally
					{
		...
		else if (indexedProperty.isLocalized() && !indexedProperty.isContextual())
		{
			...
			for (final LanguageModel language : languages)
			{
				...
				{
					i18nService.setCurrentLocale(localeService.getLocaleByString(language.getIsocode()));
					value = getPropertyValue(product, indexedProperty);
					rangeNameList = getRangeNameList(indexedProperty, value);
				}
				finally
				{
		...
		{
			final Object value = getPropertyValue(product, indexedProperty);
			rangeNameList = getRangeNameList(indexedProperty, value);
