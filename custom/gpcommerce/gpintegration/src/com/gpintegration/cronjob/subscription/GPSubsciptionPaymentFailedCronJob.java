...
public class GPSubsciptionPaymentFailedCronJob extends AbstractJobPerformable<CronJobModel> {
	...
	public PerformResult perform(final CronJobModel cronjob) {
		...
		List<GPSubscriptionCartModel> subscriptionCartModelList = subscriptionCartServiceImpl
				.getSubscriptionCartModels(SubscriptionCartStatusEnum.PAYMENTFAILURE.getCode(),true);
	...
	public Date calculateNextFrequencyDate(GPSubscriptionCartModel subscriptionCartModel) {
		...
		if(LocalDate.now().isBefore(subscriptionCartModel.getNextActivationDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate())) {
			nextDate = subscriptionCartModel.getNextActivationDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate().plusDays(frequency);
		}
		else {
			nextDate = LocalDate.now().plusDays(frequency);
		}
