 ...
 */
package com.gp.ymkt.segmentation.services;
 ...
 */
public class GPInitiativeService
{
	...
	private static final Logger LOG = LoggerFactory.getLogger(GPInitiativeService.class);
    ...
    public static final String DEFAULT_ID_ORIGIN = "SAP_HYBRIS_CONSUMER";
	...
	protected String buildFilterOption(InitiativeQuery query) throws EdmException, IOException
	{
		...
		else
		{
			...
			if (query.filterByUserContext)
			{
				...
				contactOrigins.add(getContactIdOrigin());
			}
		...
		LOG.debug( " Campaign predicate  {} " , predicate.toExpression());
	...
	public String getInteractionContactId()
	{
		...
		{
			userId =  userContextService.isAnonymousUser() ? userContextService.getAnonymousUserId() : ((CustomerModel) this.userService.getCurrentUser())
					.getOriginalUid();
	...
	public String getContactIdOrigin()
	{
		if (!userContextService.isIncognitoUser())
		{
			return userContextService.isAnonymousUser() ? Config.getString("sapymktcommon.user.origin.SAP_HYBRIS_CONSUMER", DEFAULT_ID_ORIGIN) : getOriginIdSapHybrisConsumer();
		}
	...
	private String getOriginIdSapHybrisConsumer() {
		final String siteIdOrigin = cmsSiteService.getSiteConfig(GpintegrationConstants.YMARKETING_SOURCE_SYSTEM_ID);
        final String idOrigin = siteIdOrigin != null ? siteIdOrigin : DEFAULT_ID_ORIGIN;
