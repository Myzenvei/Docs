...
public class GPInitiativeServiceB2B extends GPInitiativeService
{
	private static final Logger LOG = LoggerFactory.getLogger(GPInitiativeServiceB2B.class);
	...
	public String getInteractionContactId()
	{
		...
		if (filterOnB2BUnit)
		{
			String response = Optional.ofNullable(b2bUnitService.getParent(b2bCustomer)) //
					.map(B2BUnitModel::getUid) //
					.orElse(super.getInteractionContactId());
			return response;
		}
		String response = Optional.ofNullable(b2bCustomer).map(B2BCustomerModel::getOriginalUid) //
				.orElse(super.getInteractionContactId());


		return Optional.ofNullable(b2bCustomer).map(B2BCustomerModel::getOriginalUid).orElse(super.getInteractionContactId());
	}
