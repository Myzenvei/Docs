...
public class SubprocessesCompletedAction extends AbstractSimpleDecisionAction<OrderProcessModel>
{
	...
	public Transition executeAction(final OrderProcessModel process)
	{
		if(LOG.isDebugEnabled()){
			LOG.debug(PROCESS_MSG + process.getCode() + " in step " + getClass());
			LOG.debug(PROCESS_MSG + process.getCode() + " is checking for  " + process.getConsignmentProcesses().size()
					+ " subprocess results");
		}
		...
		if (CollectionUtils.isNotEmpty(consignmentProcesses))
		{
			...
			{
				if(LOG.isDebugEnabled()){
					LOG.debug(PROCESS_MSG + process.getCode() + " found all subprocesses incomplete");
				}
			...
			{
				if(LOG.isDebugEnabled()){
					LOG.debug(PROCESS_MSG + process.getCode() + " found subprocess " + atleastOneConsProcessNotDone.get().getCode()
		...
		if(LOG.isDebugEnabled()){
			LOG.debug(PROCESS_MSG + process.getCode() + " found all subprocesses complete");
