 ...
 */
package com.gp.commerce.cockpits.cmscockpit.components.contentbrowser;
...
import com.gp.commerce.cockpits.cmscockpit.session.impl.DefaultCmsPageBrowserModel;
	...
	protected void liveEditBtnAction() {
		...
		if (browserModel instanceof DefaultCmsPageBrowserModel)
        {
                    ...
                    && cmsPageBrowserModel.getActiveCatalogVersion() != null)
            {
                ...
                String urlBuilder = 
                			 "?" + PERSP_TAG + "=" + LIVE_EDIT_PERSPECTIVE_ID + "&" + EVENTS_TAG + "=" + LIVE_EDIT_PAGE_NAVIGATION_EVENT +
                			 "&" + LIVE_EDIT_SITE + "=" + cmsPageBrowserModel.getActiveSite().getPk().toString() + "&" + LIVE_EDIT_CATALOG +
                			 "=" + cmsPageBrowserModel.getActiveCatalogVersion().getPk().toString() + "&" + LIVE_EDIT_PAGE + "=" + pagePk.getLongValueAsString();
                if (LOG.isDebugEnabled())
                {
                    LOG.debug("URL for Open in live edit page navigation event: " + urlBuilder);
                }

                Executions.getCurrent().sendRedirect(urlBuilder);
            }
            else
            {
