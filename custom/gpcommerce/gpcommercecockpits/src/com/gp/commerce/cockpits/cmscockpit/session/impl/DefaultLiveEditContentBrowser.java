 ...
 */
package com.gp.commerce.cockpits.cmscockpit.session.impl;
...
import com.gp.commerce.cockpits.components.liveedit.DefaultLiveEditView;
	...
	protected void processOpenInPageEditButton(final Hbox hbox, final DefaultLiveEditBrowserModel liveEditBrowserModel,
		...
		if (StringUtils.isNotBlank(liveEditBrowserModel.getRelatedPagePk()))
        {
            createRightCaptionButton(Labels.getLabel("browser.openInPageEdit"), "btnliveeditcontent_pageedit", hbox,
					event -> {
                        String urlBuilder = "?" + PERSP_TAG + "=" + PAGE_VIEW_PERSPECTIVE_ID + "&" + EVENTS_TAG +
                        "=" + CMS_NAVIGATION_EVENT + "&" + CMS_PNAV_SITE + "=" + sitePk + "&" + CMS_PNAV_CATALOG + "=" + 
                        catalogPk + "&" + CMS_PNAV_PAGE + "=" + liveEditBrowserModel.getRelatedPagePk();
                        if (LOG.isDebugEnabled())
                        {
                            LOG.debug("URL for Open in Page edit event: " + urlBuilder);
                        }
