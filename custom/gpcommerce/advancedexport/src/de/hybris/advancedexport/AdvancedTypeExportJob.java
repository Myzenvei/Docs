...
import de.hybris.advancedexport.model.AdvancedTypeExportCronJobModel;import de.hybris.platform.cronjob.enums.CronJobResult;
...
public class AdvancedTypeExportJob extends AbstractJobPerformable<AdvancedTypeExportCronJobModel>
{
		...
		if (StringUtils.isNotBlank(exportDir))
		{
			...
			try
			{
				...
				else if (dir.isDirectory() && dir.canWrite())
				{
					...
					LOG.info("copy exported data " + result.getExportedData());
					copyExportedMediaFile(dirPath, result.getExportedMedia());
					LOG.info("copy exported media " + result.getExportedMedia());
				}
				else
				{
	...
	private String findRealMediaPath(final ImpExMediaModel impexModel)
	{
		...
		sb.append(configurationService.getConfiguration().getProperty("media.replication.dirs")).append(separatorChar);
