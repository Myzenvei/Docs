...
<%--This tag should use only with bazaarvoiceProductListerGridItem. You should not use it with something else elements--%>
<c:forEach items="${actions}" var="action" varStatus="idx">
	<c:if test="${action.visible}">
		<${element} class="ProductGridComponent-${fn:escapeXml(action.uid)}" data-index="${idx.index + 1}" class="${styleClass}">
