...
public class CheckoutController extends AbstractCheckoutController
{
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		Map<String, Double> productCodeToPriceWithoutTax= new HashMap<>();
		if (orderDetails.getEntries() != null && !orderDetails.getEntries().isEmpty())
		{
			for (final OrderEntryData entry : orderDetails.getEntries()) {
				...
				//todo: get price without tax correctly
//				if (!orderDetails.isNet()) {
//					ProductModel productModel = productService.getProductForCode(productCode);
//
//					Product sourceProduct = (Product) modelService.getSource(productModel);
//					boolean net = true;
//					try {
//						List<PriceInformation> priceInformations = sourceProduct.getPriceInformations(net);
//						double priceValue = priceInformations.get(0).getPriceValue().getValue();
//						productCodeToPriceWithoutTax.put(productCode, priceValue);
//					} catch (Exception e) {
//						e.printStackTrace();
//					}
//				}
        ...
        /*BAZAARVOICE_MODIFICATION_START*/
		model.addAttribute("productCodeToPriceWithoutTax", productCodeToPriceWithoutTax);
        /*BAZAARVOICE_MODIFICATION_END*/
