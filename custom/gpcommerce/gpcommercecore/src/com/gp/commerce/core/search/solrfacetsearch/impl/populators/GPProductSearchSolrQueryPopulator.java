...
public class GPProductSearchSolrQueryPopulator<INDEXED_PROPERTY_TYPE, INDEXED_TYPE_SORT_TYPE> implements
Populator<SearchQueryPageableData<SolrSearchQueryData>, SolrSearchRequest<FacetSearchConfig, IndexedType, INDEXED_PROPERTY_TYPE, SearchQuery, INDEXED_TYPE_SORT_TYPE>>
{
	...
	private static final String ITEMTYPE_STRING =  "itemType_string";
	private static final String SKU_NGRAM =  "sku_ngram";
	private static final String SKU_EDGE =  "sku_edge";
			...
			final SolrSearchRequest<FacetSearchConfig, IndexedType, INDEXED_PROPERTY_TYPE, SearchQuery, INDEXED_TYPE_SORT_TYPE> target)
	{
		...
		if(null !=source.getSearchQueryData() && CollectionUtils.isNotEmpty(source.getSearchQueryData().getFilterQueries())){
			Optional<SolrSearchFilterQueryData> filterQuery = source.getSearchQueryData().getFilterQueries().stream().filter(i -> i.getKey().equalsIgnoreCase(ITEMTYPE_STRING)
					&& i.getValues().contains(GPCompetitorProductModel._TYPECODE)).findAny();
			if(filterQuery.isPresent()){
				if(CollectionUtils.isNotEmpty(searchQuery.getFreeTextFuzzyQueries())){
					searchQuery.getFreeTextFuzzyQueries().removeIf(f -> !f.getField().equalsIgnoreCase(CODE) && !f.getField().equalsIgnoreCase(SKU_NGRAM)
							&& !f.getField().equalsIgnoreCase(SKU_EDGE)
							);
				}
				
				if(CollectionUtils.isNotEmpty(searchQuery.getFreeTextPhraseQueries())){
					searchQuery.getFreeTextPhraseQueries().removeIf(f -> !f.getField().equalsIgnoreCase(CODE) && !f.getField().equalsIgnoreCase(SKU_NGRAM)
							&& !f.getField().equalsIgnoreCase(SKU_EDGE)
							);
				}
				if(CollectionUtils.isNotEmpty(searchQuery.getFreeTextQueries())){
					searchQuery.getFreeTextQueries().removeIf(f -> !f.getField().equalsIgnoreCase(CODE) && !f.getField().equalsIgnoreCase(SKU_NGRAM)
							&& !f.getField().equalsIgnoreCase(SKU_EDGE)
							);
				}
				if(CollectionUtils.isNotEmpty(searchQuery.getFreeTextWildcardQueries())){
					searchQuery.getFreeTextWildcardQueries().removeIf(f -> !f.getField().equalsIgnoreCase(CODE) && !f.getField().equalsIgnoreCase(SKU_NGRAM)
							&& !f.getField().equalsIgnoreCase(SKU_EDGE)
							);
				}
			}
