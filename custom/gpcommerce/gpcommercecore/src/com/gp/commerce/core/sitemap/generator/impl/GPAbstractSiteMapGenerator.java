...
import com.gp.commerce.core.sitemap.renderer.GPSiteMapContext;


/**
 * Abstract Class to generate Site Maps for GP Sites
 */
public abstract class GPAbstractSiteMapGenerator<T> implements SiteMapGenerator<T>, ApplicationContextAware
{
			...
			final RendererTemplateModel rendererTemplateModel, final List<T> models, final String filePrefix,final Integer index) throws IOException
	{
		...
		return getImpersonationService().executeInContext(context, () -> {
			...
			final GPSiteMapContext siteMapContext = (GPSiteMapContext) applicationContext.getBean("gpSiteMapContext");
