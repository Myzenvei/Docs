...
public class ColorFacetValueProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider
{
	...
	{
		final GPCommerceStyleVariantProductModel gpCommerceStyleModel = getGPCommerceStyleProductModel(model);
		if (gpCommerceStyleModel == null)
		{
			return Collections.emptyList();
		}

		final Set<SwatchColorEnum> colors = gpCommerceStyleModel.getSwatchColors();
	...
	protected GPCommerceStyleVariantProductModel getGPCommerceStyleProductModel(final Object model)
	{
		Object finalModel = model;
		if (model instanceof GPCommerceSizeVariantProductModel)
		{
			finalModel = ((GPCommerceSizeVariantProductModel) finalModel).getBaseProduct();
		}

		if (finalModel instanceof GPCommerceStyleVariantProductModel)
		{
			return (GPCommerceStyleVariantProductModel) finalModel;
		}
		else
		{
			return null;
