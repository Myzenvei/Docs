...
(function(global) {
    ...
    function initNotifyButton(){
        $(".notify-me-placeholder").on('click', 'button', function (e) {
            e.preventDefault();
            e.stopImmediatePropagation();
            //Send Product Data to Vue
            vm.notifyProduct($(this).data());
        });
    }

    function initFavoriteButton(){
        var $wishlistBtn = $(".wishlist-btn");
        if(/^((?!chrome|android).)*safari/i.test(navigator.userAgent) && $wishlistBtn.find("svg").hasClass("gpemployee")){
            //Set aspect ration property for Safari if SVG is for Employee Store
            $wishlistBtn.find("svg").attr("preserveAspectRatio","none");
        }

        $wishlistBtn.on('click', 'button', function (e) {
            e.preventDefault();
            e.stopImmediatePropagation();
            // Send Product Data to Vue
            // vm.addProductToCart($(this).data());
            //TODO: Send isActive Flag and Toggle Favorites functionality
            var productObj = $.extend({},$(this).data(), {"isActive":false})
            if($(this).hasClass("active")){
                $.extend(productObj,{"isActive":true});
                vm.addProductToFavorites(productObj);
            } else {
                vm.addProductToFavorites(productObj);
            }
        });
    ...
    function initAddToCartButton() {
        $(".add-to-cart-placeholder").on('click', 'button', function (e) {
            e.preventDefault();
            e.stopImmediatePropagation();
            //Send Product Data to Vue
            vm.addProductToCart($(this).data());
    ...
    //Handler from Vue to JSP
    //Using this to attach function on Window object
    this.updateFavoriteProduct = function(productId) {
        if(productId){
            $(".wishlist-btn > button[data-code="+ $.escapeSelector(productId) +"]").toggleClass("active");
        }
    }
