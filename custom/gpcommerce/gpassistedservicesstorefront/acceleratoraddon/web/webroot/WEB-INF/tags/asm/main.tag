...
<%@ taglib prefix="asm" tagdir="/WEB-INF/tags/addons/gpassistedservicesstorefront/asm"%>
...
<div id="_asm" <c:if test="${not empty asm_message}"> style="display: block;"</c:if>>
    <script>
    	...
    	if(document.querySelectorAll('#_asm').length>1){
    		document.querySelectorAll('#_asm')[1].remove();
    	}
    </script>
	<div class="ASM_header navigation navigation--top">
        <div class="first-top-row row">
            <asm:redirect />
            <c:url value="/assisted-service/quit" var="quitURL" />
			<form action="${quitURL}" method="post" id="asmLogoutForm1"
				class="asmForm pull-right">
				<input name="CSRFToken" type="text" value="${CSRFToken.token}"
					hidden>
				<fieldset>
					<button type="submit" class="ASM_close ASM_close_all closeBtn">&times;</button>
				</fieldset>
			</form>
                ...
                <c:when test="${not empty agent.uid}">
                    <div id="_asmLogged" class="ASM_loggedin">
                        ...
                        <div class="ASM_timer element-separator-height">
                            ...
                            <c:url value="/assisted-service/resetSession" var="resetURL" />
							<form action="${resetURL}" method="post" id="asmResetForm"
								class="asmForm pull-right">
								<input name="CSRFToken" type="text" value="${CSRFToken.token}"
									hidden>
								<fieldset>
									<button type="submit" id="resetButton"
										class="ASM-btn ASM-btn-reset" disabled>
										<spring:theme code="asm.emulate.reset" />
									</button>
                        ...
                        <div class="ASM_loggedin_text element-separator-height">
                            ...
                            <form action="${logoutActionUrl}" method="post" id="asmLogoutForm" class="asmForm">
                                <input name="CSRFToken" type="text" value="${CSRFToken.token}" hidden>
                                <fieldset>
