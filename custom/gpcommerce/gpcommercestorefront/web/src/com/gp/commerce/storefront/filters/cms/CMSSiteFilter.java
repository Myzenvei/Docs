 ...
 */
package com.gp.commerce.storefront.filters.cms;
...
public class CMSSiteFilter extends OncePerRequestFilter implements CMSFilter
{
			...
			final FilterChain filterChain) throws ServletException, IOException
	{
		...
		final CmsPageRequestContextData cmsPageRequestContextData = getCmsPageContextService().initialiseCmsPageContextForRequest(
				httpRequest);
		BaseSiteModel site =getCmsSiteService().getCurrentSite();
		if(null != site && null != site.getUid() && GPEMPLOYEE.equalsIgnoreCase(site.getUid() )) {
			getSessionService().setAttribute(SITE, GPEMPLOYEE);;
		}
