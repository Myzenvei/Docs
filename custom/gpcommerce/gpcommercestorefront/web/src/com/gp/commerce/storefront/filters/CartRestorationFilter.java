...
import com.gp.commerce.storefront.security.cookie.CartRestoreCookieGenerator;
...
public class CartRestorationFilter extends OncePerRequestFilter	
{
	...
	public static final String EMP_SITE = "gpemployee";
	...
	public static final String SHARE_FAV = "sharefavourite";
	...
	private GPSessionHelperService sessionHelperService;

	@Override
	public void doFilterInternal(final HttpServletRequest request, final HttpServletResponse response,
			final FilterChain filterChain) throws IOException, ServletException
	{
		//set the asm session var
		sessionHelperService.updateASMSession();

		if (getUserService().isAnonymousUser(getUserService().getCurrentUser()))
		{
			if(EMP_SITE.equalsIgnoreCase(getBaseSiteService().getCurrentBaseSite().getUid())){
				String[] listId = null;
				if(request.getRequestURI().contains(SHARE_FAV)) {
					for (Entry<String, String[]> entry : request.getParameterMap().entrySet()) {
						if(LIST_NAME.equalsIgnoreCase(entry.getKey())) {
							listId = entry.getValue();
						}
					}
					if(listId != null && listId[0] != null) {
						Cookie cookie = new Cookie("share",listId[0]);
				...
				if(request.getRequestURI().contains(CART) || request.getRequestURI().contains(CHECKOUT) || request.getRequestURI().contains(CATEGORIES) || request.getRequestURI().contains(SEARCH) || request.getRequestURI().contains(PRODUCTS) ||  request.getRequestURI().contains(DIXIE) ||  request.getRequestURI().contains(SHARE_FAV)) {
	...
	public GPSessionHelperService getSessionHelperService() {
		return sessionHelperService;
	}

	public void setSessionHelperService(GPSessionHelperService sessionHelperService) {
