...
public class CheckoutLoginController extends AbstractLoginPageController
{
			...
			final Model model, final HttpServletRequest request, final HttpServletResponse response) throws CMSItemNotFoundException
	{
		if(null!=request.getCookies()) {
			Cookie cookieValue =  Arrays.asList(request.getCookies()).stream().filter(cookie -> "guid".equalsIgnoreCase(cookie.getName())).findFirst().orElse(null);
			if(null != cookieValue) {
				CartModel cart=getCommerceCartService().getCartForGuidAndSite(cookieValue.getValue(), getBaseSiteService().getCurrentBaseSite());
				if(null != cart) {
		...
		String redirect = super.processAnonymousCheckoutUserRequest(form, bindingResult, model, request, response);
