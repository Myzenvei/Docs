...
public class WebHttpSessionRequestCache extends HttpSessionRequestCache implements Serializable
{
	...
	private static final String GPEMPLOYEE = "gpemployee";
			...
			if (getRequestMatcher().matches(request))
			{
				final DefaultSavedRequest savedRequest = new DefaultSavedRequest(request, getPortResolver())
				{
					...
					public String getRedirectUrl()
					{
						...
						if(null != getSessionService().getAttribute(SITE)) {
							site= getSessionService().getAttribute(SITE).toString();
						}
						return calculateRelativeRedirectUrl(contextPath, referer,site);
					}
				};
	...
	protected String calculateRelativeRedirectUrl(final String contextPath, final String url,String site)
	{
		if(null != site && GPEMPLOYEE.equals(site)) {
			return "/";
		}
