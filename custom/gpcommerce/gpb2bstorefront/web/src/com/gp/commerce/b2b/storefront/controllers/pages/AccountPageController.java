...
public class AccountPageController extends AbstractSearchPageController
{
	...
	private static final String SUPPORT_COUNTRY_ATTR = "supportedCountries";
	private static final String EMAIL_ATTR = "updateEmailForm";
	private static final String MULTI_MAP_ATTR = "readOnlyMultiDMap";
	private static final String VALIDATION_EMAIL = "validation.checkEmail.equals";
	private static final String EMAIL_UNIQUE = "profile.email.unique";
	private static final String PROFILE_UPDATED = "text.account.profile.confirmationUpdated";
	private static final String CHECK_EMAIL = "chkEmail";
	private static final String PROFILE_ATTR = "updateProfileForm";
	private static final String PASSWORD_ATTR = "updatePasswordForm";
	private static final String CURRENT_PASSWORD = "currentPassword";
	private static final String NEW_PASSWORD = "checkNewPassword";
	private static final String CUSTOMER_ATTR = "customerData";
	private static final String CONSENT_ATTR = "consentTemplateDataList";
	...
	{
		model.addAttribute(SUPPORT_COUNTRY_ATTR, getCountries());
			...
			@RequestParam("productCode") final String productCode, final Model model)
	{
		...
		model.addAttribute(MULTI_MAP_ATTR, readOnlyMultiDMap);
	...
	public String editEmail(final Model model) throws CMSItemNotFoundException
	{
		...
		model.addAttribute(EMAIL_ATTR, updateEmailForm);
			...
			final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		{
			bindingResult.rejectValue(CHECK_EMAIL, VALIDATION_EMAIL, new Object[] {}, VALIDATION_EMAIL);
		}
		...
		else
		{
			...
			{
				bindingResult.rejectValue("email", EMAIL_UNIQUE);
				returnAction = setErrorMessagesAndCMSPage(model, UPDATE_EMAIL_CMS_PAGE);
			}
	...
	public String editProfile(final Model model) throws CMSItemNotFoundException
	{
		...
		model.addAttribute(PROFILE_ATTR, updateProfileForm);
	...
	public String updatePassword(final Model model) throws CMSItemNotFoundException
	{
		...
		model.addAttribute(PASSWORD_ATTR, updatePasswordForm);
			...
			final Model model, final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		if (!bindingResult.hasErrors())
		{
			...
			{
				bindingResult.rejectValue(NEW_PASSWORD, "validation.checkPwd.equals", new Object[] {},
	...
	{
		model.addAttribute(CUSTOMER_ATTR, customerFacade.getCurrentCustomer());
		model.addAttribute(PASSWORD_ATTR, userFacade.getCCPaymentInfos(true));
	...
	{
		model.addAttribute(CONSENT_ATTR, getConsentFacade().getConsentTemplatesWithConsents());
	...
	public String getPaymentDetails(final Model model) throws CMSItemNotFoundException
	{
			...
			throws CMSItemNotFoundException
	{
		...
		final List<Breadcrumb> breadcrumbs = accountBreadcrumbBuilder.getBreadcrumbs(null);
		if(cmsSiteService.getCurrentSite() !=null && cmsSiteService.getCurrentSite().getUid().equalsIgnoreCase(getConfigurationService().getConfiguration().getString(GpcommerceCoreConstants.GPXPRESS_SITE_ID)))
		{
			breadcrumbs.add(new Breadcrumb(MY_ACCOUNT_LISTS_PAGE_URL, getMessageSource().getMessage("text.account.lists", null,
	                getI18nService().getCurrentLocale()), null));
		}
		else
		{
		...
		if(anonymousCookie !=null && ANONYMOUS.equalsIgnoreCase(anonymousCookie.getValue())) {
			isAnonymous = true;
		}
