 ...
 */
package com.gp.commerce.b2b.storefront.interceptors.beforecontroller;
...
import com.gp.commerce.b2b.storefront.security.evaluator.impl.RequireHardLoginEvaluator;
...
public class RequireHardLoginBeforeControllerHandler implements BeforeControllerHandler
{
			...
			final HandlerMethod handler) throws Exception
	{
		...
		if (request.isSecure())
		{
			...
			if (annotation != null)
			{
				...
				if (redirect)
				{
					if(request.getServletPath().contains("subscriptions") || (request.getServletPath().contains("order")))
					{
						HttpSession session = request.getSession(true);
						String redirectURL=request.getRequestURI().toString().replace(request.getContextPath(), "");
						session.setAttribute("subscReferer", redirectURL);
					}
	...
	{
		if (request != null
				&& request.getServletPath().contains("checkout"))
		{
