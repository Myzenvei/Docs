...
public class QuickOrderPageController extends AbstractPageController
{
	...
	public static final String REDIRECT_TO_ERROR = REDIRECT_PREFIX + "/quick";
	...
	{
		String isShowQuickOrder = cmsSiteService.getSiteConfig("isShowQuickOrder");
		if(isShowQuickOrder == null || "false".equalsIgnoreCase(isShowQuickOrder)) {
			return REDIRECT_TO_ERROR;
		}
		storeCmsPageInModel(model, getContentPageForLabelOrId("quickOrderPage"));
		model.addAttribute("quickOrderMinRows", Integer.valueOf(Config.getInt("gpb2bstorefront.quick.order.rows.min", 3)));
		model.addAttribute("quickOrderMaxRows", Integer.valueOf(Config.getInt("gpb2bstorefront.quick.order.rows.max", 25)));
