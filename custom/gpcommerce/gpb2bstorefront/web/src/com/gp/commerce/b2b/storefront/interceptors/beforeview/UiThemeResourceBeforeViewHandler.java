 ...
 */
package com.gp.commerce.b2b.storefront.interceptors.beforeview;
...
import com.gp.commerce.b2b.storefront.web.view.UiExperienceViewResolver;
...
public class UiThemeResourceBeforeViewHandler implements BeforeViewHandler {
	...
	protected static final String CLUDO_SEARCH_SITE_KEY = "cludo.site.key";
	protected static final String CLUDO_SEARCH_URL = "cludo.search.url";
	protected static final String CLUDO_AUTOSUGGEST_URL = "cludo.autosuggest.url";
	protected static final String SAMPLECART_ENABLE = "enable.samplecart";
	protected static final String ORDER_SAMPLE_REDIRECT_URL = "samplecart.order.redirecturl";
	...
	public void beforeView(final HttpServletRequest request, final HttpServletResponse response,
		...
		modelAndView.addObject("cludoSiteKey", Config.getParameter(CLUDO_SEARCH_SITE_KEY));
		modelAndView.addObject("cludoSearchURL", Config.getParameter(CLUDO_SEARCH_URL));
		modelAndView.addObject("cludoAutoSuggestURL", Config.getParameter(CLUDO_AUTOSUGGEST_URL));
		modelAndView.addObject("enablesamplecart", cmsSiteService.getSiteConfig(SAMPLECART_ENABLE));
		modelAndView.addObject("orderSampleCartRedirectUrl",Config.getParameter(ORDER_SAMPLE_REDIRECT_URL));
		...
		if(StringUtils.isNotEmpty(cmsSiteService.getSiteConfig(SHOW_TAX_EXEMPTION)) && Boolean.valueOf(cmsSiteService.getSiteConfig(SHOW_TAX_EXEMPTION))) {
			modelAndView.addObject(SHOW_TAX_EXEMPTION, true);
		}else {
			modelAndView.addObject(SHOW_TAX_EXEMPTION, false);
		}

		Boolean showCompanyLinks = Boolean.FALSE;
		if(StringUtils.isNotEmpty(cmsSiteService.getSiteConfig(SHOW_COMPANY_LINKS)) && Boolean.valueOf(cmsSiteService.getSiteConfig(SHOW_COMPANY_LINKS))) {
			showCompanyLinks = Boolean.TRUE;
		}
		
		if ((currentCustomer instanceof B2BCustomerModel) && gpB2BUnitsService.isBuyer((B2BCustomerModel) currentCustomer)) {
			modelAndView.addObject(SHOW_COMPANY_LINKS, Boolean.FALSE);

		} else {
			modelAndView.addObject(SHOW_COMPANY_LINKS, showCompanyLinks);
		...
		/**Added code to configure download/share icons on tool bar**/
		...
		boolean isCheckoutEnabled = null !=cmsSiteService.getSiteConfig(IS_CHECKOUT_ENABLED) ?Boolean.valueOf(cmsSiteService.getSiteConfig(IS_CHECKOUT_ENABLED)) :false;
		modelAndView.addObject(IS_CHECKOUT_ENABLED, isCheckoutEnabled);
	...
	private NavigationUrlData getNavigationUrls() {
		...
		navigations.setOrderApprovalDetails(GpcommerceFacadesConstants.ORDERAPPROVAL_DETAILS_PAGE_URL);
		...
		navigations.setLeasePage(GpcommerceFacadesConstants.LEASE);
		navigations.setCategory(GpcommerceFacadesConstants.CATEGORY);
	...
	protected void addAddOnResourcePaths(final String contextPath, final List<String> addOnResourcePaths,
			final String addon, final String propertyName) {
