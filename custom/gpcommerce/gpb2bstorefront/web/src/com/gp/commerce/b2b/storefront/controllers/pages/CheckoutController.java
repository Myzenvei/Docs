...
public class CheckoutController extends AbstractCheckoutController
{
	...
	private static final String IS_CHECKOUT_ENABLED="isCheckoutEnabled";
	...
	public String checkout(final Model model, final RedirectAttributes redirectModel,final HttpServletRequest request) throws CMSItemNotFoundException
	{
		...
		{
			model.addAttribute("guestEmail", getSessionService().getAttribute(GUEST_EMAIL));
		}
		return getView();
	}

	private String getView() {
		if ((!getUserFacade().isAnonymousUser()) || (StringUtils.isNotEmpty(cmsSiteService.getSiteConfig(IS_CHECKOUT_ENABLED)) && Boolean.valueOf(cmsSiteService.getSiteConfig(IS_CHECKOUT_ENABLED)))) {
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		//Adding order code for clickstream events
