...
public class PasswordResetPageController extends AbstractPageController
{
	...
	private static final String UPDATE_PWD_LINK_EXPIRE_CMS_PAGE = "updatePasswordLinkExpire";
	...
	public String getChangePassword(@RequestParam(required = false) final String token, final Model model)
			throws CMSItemNotFoundException, IllegalArgumentException, TokenInvalidatedException
	{
		...
		if(isValidToken && null != customer.getToken()){
			...
			storeCmsPageInModel(model, getContentPageForLabelOrId(UPDATE_PWD_LINK_EXPIRE_CMS_PAGE));
			setUpMetaDataForContentPage(model, getContentPageForLabelOrId(UPDATE_PWD_CMS_PAGE));
			model.addAttribute(new ForgottenPwdForm());
